...
| `riscv_sw/` | **The core runs real C.** `hello.c` (freestanding, `-march=rv32im -O2`, no libc/libgcc) + `start.S` + `link.ld` (ROM `0x0000_0000`, RAM `0x1000_0000`, 64 KB each) build into `hello.hex` with a stock `riscv64-unknown-elf-gcc` (Debian package, Docker recipe in `riscv_sw/README.md`); `hex2volt.py` turns the hex into `hello_rom.volt`, a `const` word table behind a two-port ROM module (ADR-0041 variable-index const array). `hello_soc.volt` wraps `RiscvCore` with that ROM, a 64-word RAM and an 8N1 receiver on `uart_txd` that stores every byte; the test pages through the buffer with `rx_sel`, because test blocks can only set ports, step and assert (no arrays, loops or files — which is also why the program is a generated Volt file instead of something the test loads). `hello program prints greeting` in `riscv_core_test.volt`: `Hello from Volt!\n42\n` arrives byte for byte, 847 instructions in 913 cycles (20 UART frames of 40 cycles dominate; the DIV and the REM stall 33 cycles each), no trap, no stray bus access; `42` is `7 * 6` through MUL, its digits through DIV and REM on `volatile` operands so `-O2` cannot fold them. A `+ 1` mutant on the multiplier output prints `43` and fails the test. Both generated files are committed: `volt test` needs no RISC-V toolchain. Verilator `-Wall` clean (HelloSoc + HelloRom + RiscvCore + UartTx). |
