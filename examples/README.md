...
| `i2c/` | I2C master (see [`i2c/README.md`](i2c/README.md)) — the design that got `inout` writing and the `opendrain` port kind into the language (ADR-0051): 100/400 kHz, 7-bit addressing, single-byte write/read, ACK/NACK, START / STOP / REPEATED START, clock stretching. `opendrain sda`/`scl` pads driven with `sda.drive_low()` / `sda.release()`, read through `sync(sda.read(), clk)` (a direct read is W3007), contracts on the drive intent (`sda.released`). Pure Volt: the compiler emits `assign sda = sda_drive_low ? 1'b0 : 1'bz`; the test bench binds master and slave to two `wire`s that become `tri1` nets. 7-state FSM on an `enum I2cState` register as an exhaustive `match` in the `on clk` block (the compiler adds the state-valid invariant for the unused code 7), 4-phase bit divider from `const CLKS_PER_BIT_*` (3.2 MHz: 32 / 8 clocks per bit). Verilator `-Wall` clean (tri1 + 1'bz), 12/12 tests, 13 properties `bmc 12` / `prove 3 --engine boolector`, 7/7 covers at `cover 190`. |
