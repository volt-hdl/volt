...
use axi4lite_slave::{AxiAddr, AxiWData, AxiResp, AxiRData, Axi4LiteSlave};
...
pub module SocTop {
    ...
    let gpio_i = Gpio {
        clk: clk,
        aw_addr: off_aw_addr, aw_prot: aw.data.prot, aw_valid: gpio_aw_valid,
        w_data: w.data.data, w_strb: w.data.strb, w_valid: gpio_w_valid,
        b_ready: gpio_b_ready,
        ar_addr: off_ar_addr, ar_prot: ar.data.prot, ar_valid: gpio_ar_valid,
        r_ready: gpio_r_ready,
        pins_in: gpio_in,
    }
    let timer_i = Timer {
        clk: clk,
        aw_data_addr: off_aw_addr, aw_data_prot: aw.data.prot, aw_valid: timer_aw_valid,
        w_data_data: w.data.data, w_data_strb: w.data.strb, w_valid: timer_w_valid,
        b_ready: timer_b_ready,
        ar_data_addr: off_ar_addr, ar_data_prot: ar.data.prot, ar_valid: timer_ar_valid,
        r_ready: timer_r_ready,
    }
    let uart_i = UartCtrl {
        clk: clk,
        aw_data_addr: off_aw_addr, aw_data_prot: aw.data.prot, aw_valid: uart_aw_valid,
        w_data_data: w.data.data, w_data_strb: w.data.strb, w_valid: uart_w_valid,
        b_ready: uart_b_ready,
        ar_data_addr: off_ar_addr, ar_data_prot: ar.data.prot, ar_valid: uart_ar_valid,
        r_ready: uart_r_ready,
    }
    let sys_id : u32 = 0x50C00001
    let sys_i = Axi4LiteSlave {
        clk: clk,
        aw_data_addr: off_aw_addr, aw_data_prot: aw.data.prot, aw_valid: sys_aw_valid,
        w_data_data: w.data.data, w_data_strb: w.data.strb, w_valid: sys_w_valid,
        b_ready: sys_b_ready,
        ar_data_addr: off_ar_addr, ar_data_prot: ar.data.prot, ar_valid: sys_ar_valid,
        r_ready: sys_r_ready,
        status: sys_id,
    }
    ...
    let dec_i = BusDecoder {
        clk: clk,
        aw_addr: aw.data.addr,
        ar_addr: ar.data.addr,
        ...
        timer_b_valid: timer_i.b_valid, timer_b_resp: timer_i.b_data_resp,
        timer_ar_ready: timer_i.ar_ready, timer_r_valid: timer_i.r_valid,
        timer_r_data: timer_i.r_data_data, timer_r_resp: timer_i.r_data_resp,
        uart_aw_ready: uart_i.aw_ready, uart_w_ready: uart_i.w_ready,
        uart_b_valid: uart_i.b_valid, uart_b_resp: uart_i.b_data_resp,
        uart_ar_ready: uart_i.ar_ready, uart_r_valid: uart_i.r_valid,
        uart_r_data: uart_i.r_data_data, uart_r_resp: uart_i.r_data_resp,
        sys_aw_ready: sys_i.aw_ready, sys_w_ready: sys_i.w_ready,
        sys_b_valid: sys_i.b_valid, sys_b_resp: sys_i.b_data_resp,
        sys_ar_ready: sys_i.ar_ready, sys_r_valid: sys_i.r_valid,
        sys_r_data: sys_i.r_data_data, sys_r_resp: sys_i.r_data_resp,
    }
    ...
    b.data.resp = dec_i.host_b_resp
    ar.ready    = dec_i.host_ar_ready
    r.valid     = dec_i.host_r_valid
    r.data.data = dec_i.host_r_data
    r.data.resp = dec_i.host_r_resp
