...
// (`dut.aw_data_addr`, `dut.b_valid`, ...). Inputs hold their value between
...
test "full-word write reaches reg1 and completes with b.valid" {
    let dut = Axi4LiteSlave { };
    dut.aw_data_addr = 0x4;
    dut.aw_valid = true;
    dut.w_data_data = 0xDEADBEEF;
    dut.w_data_strb = 0xF;
    dut.w_valid = true;
    step(1);
    dut.aw_valid = false;
...
test "byte strobes mask the written bytes" {
    let dut = Axi4LiteSlave { };
    dut.aw_data_addr = 0x8;
    dut.aw_valid = true;
    dut.w_data_data = 0x11223344;
    dut.w_data_strb = 0x3;
    dut.w_valid = true;
    step(1);
    dut.w_data_data = 0xAABBCCDD;
    dut.w_data_strb = 0xC;
    dut.b_ready = true;
    step(1);
...
test "read returns the written register and holds until r.ready" {
    let dut = Axi4LiteSlave { };
    dut.aw_data_addr = 0xC;
    dut.aw_valid = true;
    dut.w_data_data = 0x0BADF00D;
    dut.w_data_strb = 0xF;
    dut.w_valid = true;
    dut.b_ready = true;
    step(1);
    dut.aw_valid = false;
    dut.w_valid = false;
    dut.ar_data_addr = 0xC;
    dut.ar_valid = true;
    step(1);
    dut.ar_valid = false;
    assert_true(dut.r_valid);
    assert_eq(dut.r_data_data, 0x0BADF00D);
    assert_false(dut.ar_ready);
    step(2);
    assert_true(dut.r_valid);
    dut.r_ready = true;
