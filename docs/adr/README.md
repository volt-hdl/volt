...
| [0070](ADR-0070-tani-tutarliligi.md) | Tanı tutarlılığı: `check`, LSP ve `build` aynı tanıları üretir — tek kapılı boru hattı (`volt_hir::run_semantic_stages`), `check`/LSP çıktısız emit doğrulaması, LSP birim yükleme + katlama + üst sınır 200, kalıcı parite testi, takma ad SV çözümü, E0003 metinleri; A sınıfı listesi | K | 2026-09-24 |
| [0071](ADR-0071-extern-sv-eslemesi.md) | `extern module` örneklemesinin SV eşlemesi — bildirilen portlar adıyla, örtük reset portu yok, generic extern E0003; ui/pass fixture'ları build'den de geçer (`//~ CHECK-ONLY: <neden>` istisnası) | K | 2026-09-24 |
| [0072](ADR-0072-kucuk-tani-duzeltmeleri.md) | Küçük tanı düzeltmeleri — açılmış `for` gövdesinde kaynak adı (W1001/E1003/E4011 katlanır), E2005 bölündü (YENİ E4011 örnek portu bağlantısı; E2021/E2027/E2003), `sync`/`sync3` argüman sayısı E2003, ölü reset zinciri üretilmez/kısıtlanmaz, okunmayan örnek çıkışı Verilator susturması | K | 2026-09-24 |
| [0073](ADR-0073-coklu-surucu-denetimi.md) | Çoklu sürücü denetimi — tek tablo, sürücü türleri (atama, `let` başlangıcı, giriş portu, inout/opendrain hattı), kısmi hedeflerde bit aralığı kesişimi; 22 sessiz çift sürücü yolu E4001 | K | 2026-09-24 |
| [0074](ADR-0074-enum-destegi.md) | Enum desteği (tasarım) — birim varyantlı enum donanıma iner: ikili `clog2` kodlama, açık değer + taban tipi (hepsi ya da hiçbiri), `uN → enum` yasak, sıralama E2003, kapsayıcı `match`'te `_` isteğe bağlı (son kol `default`), SV `localparam` (paket reddedildi: Yosys dosya sırası), enum FSM'de F1; payload/one-hot/@mmio ertelendi | K | 2026-09-24 |
| [0075](ADR-0075-yaniltici-rapor-ve-tani-temizligi.md) | Yanıltıcı raporlar ve tanı kalitesi — `verify` sby'nin beş durumu (YENİ E5002 kanıtlanamadı/çıkış 7, zaman aşımı/çıkış 8, `--timeout`), W4001/W4002 ayrılmış (W1001/W1004 tek bulgu), kapı arkasında enum E0014, sayısal yinelenen kol W2014, bundle alanı kaynak adıyla, aşama adı kalıntıları | K | 2026-09-24 |
| [0076](ADR-0076-extern-kaynaklari.md) | extern modül kaynakları — `@source("...")` niteliği (Volt.toml listesi reddedildi), `run`/`test`/`verify` gövdeyi araca verir, YENİ E1012; extern içi CDC için SDC kısıtı üretilmez (raporlanır, gizlenmez); flop'suz modülün reset portu kalır, Verilator susturulur | K | 2026-09-24 |
| [0077](ADR-0077-struct-destegi.md) | Struct desteği (tasarım) — düz struct değeri donanıma iner: `struct` ve `struct port` ayrı kalır, bit düzeni ilk alan MSB (SV packed), literalde bütün alanlar zorunlu, sıralama E2003, enum/`Trit` yapraklı `bits → struct` yasak, SV'de alanlar ayrı sinyallere açılır (paket reddedildi: Yosys paketler arası tip/`import`/`'{…}` ve dosya sırası; düz vektör reddedildi: İ1, VCD), struct dizisi ertelendi (SoA 5× BRAM ölçüldü); Handshake/@mmio değişmez | K | 2026-09-24 |
...
| [0080](ADR-0080-ozyineleme-derinligi.md) | Özyineleme derinliği — parser ağaç yüksekliğini sınırlar (`MAX_DEPTH` 256; operatör, `.`/`[]`/`as` ve `else if` zincirinde halka başına bir kat; tip takma adı/struct zinciri açılmış derinlikle), sınırda tek E0018 ve grup atlama (kaskad yok); derleyici 64 MB'lık bilinen yığında koşar (`main`, LSP işçileri, `parse`); ADR-0068 §6'daki yığın taşması (abort) bulgusu | K | 2026-09-25 |
| [0081](ADR-0081-fonksiyon-destegi.md) | Fonksiyon desteği (tasarım + uygulama: E2015 sonuç yok, E2016 kombinasyonel değil, E3015 fn'de declassify, E4013 özyineleme) — `fn` saf kombinasyonel (durum, yan etki, erken `return`, `declassify` yok; dönüş tipi ve son ifade zorunlu), gövde imzaya karşı bir kez denetlenir, çağrıda domain/trust/timing argümanların join'i, özyineleme çağrı çizgesinde reddedilir (ADR-0069 Tarjan'ı ortak yardımcıya), açılım ADR-0068 bütçesiyle; SV'de çağrı yerinde açılım, ara değerler `<fn>_<k>_<let>` telleri (modül içi `function` reddedildi: Verilator VARHIDDEN / bit düzeyi UNUSEDSIGNAL / ad çakışması, Yosys `return`, bind'li SVA, VCD ve kararsız netlist adları; paket reddedildi: dosya sırası) — üç biçim 129/129 `$equiv` eşdeğer; generic fn, const/test bağlamı, fn kontratları ertelendi | K | 2026-09-26 |
| [0068](ADR-0068-tani-katlama-ve-ust-sinir.md) | Açılan kopyalarda özdeş tanı katlama (`for`/mono/bundle dizisi — "reported once; occurs in N …" notu), W0023 + `--max-diagnostics` (1000), açılım düğüm bütçesi (§6: `MAX_EXPANSION_NODES` — `for` + mono klonlarının TÜM yazımları, yazma kapısı `AstWriter`, fuzz'ın üçüncü bulgusu); fuzz'ın ikinci bulgusu, ADR-0067 §4'ü düzeltir | K | 2026-09-23 |
