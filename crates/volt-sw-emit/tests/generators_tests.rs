...
fn c_header_has_guard_includes_and_base() {
    ...
    // ADR-0063: iki imza satırı en üstte, ADR-0053 blok yorumu hemen ardından.
    let hash = volt_sw_emit::check::regmap_hash(&timer());
    assert!(h.starts_with(&format!(
        "// Generated by Volt 0.1.0 from timer.volt\n// regmap-hash: {hash}\n/* `TimerRegs` register map"
    )));
