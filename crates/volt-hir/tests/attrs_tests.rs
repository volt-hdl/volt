...
#[test]
fn timing_on_module_warns_w0021() {
    let d = diags(&with_attr("@timing(clk = 25175000)"));
    assert_eq!(count(&d, "W0021"), 1, "{d:?}");
    let w = d.iter().find(|d| d.code.as_str() == "W0021").unwrap();
    assert!(
        w.message.contains("@timing"),
...
#[test]
fn false_path_on_module_warns_w0021() {
    let d = diags(&with_attr("@false_path(from = a, to = y)"));
    assert_eq!(count(&d, "W0021"), 1, "{d:?}");
}
...
fn allow_unenforced_on_same_node_silences() {
    let d = diags(&with_attr("@timing(clk = 25175000) @allow(unenforced)"));
    assert_eq!(count(&d, "W0021"), 0, "{d:?}");
...
fn allow_with_unknown_argument_is_e0009() {
    let d = diags(&with_attr("@timing(clk = 1) @allow(unenforcd)"));
...
fn timing_reason_mentions_sdc() {
    ...
    let reason = w
        ...
        .map(|n| n.text.as_str())
        .unwrap_or("");
    assert!(reason.contains("SDC"), "{reason}");
}

#[test]
fn lint_allow_policy_silences_everything() {
    let ast = parse_clean(&with_attr("@timing(clk = 1)\n@budget(lut = 1)"));
