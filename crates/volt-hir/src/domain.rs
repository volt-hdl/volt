...
impl<'a> Inferencer<'a> {
    ...
    fn bidir_read(&mut self, def: DefId, span: Span) -> DomainId {
    ...
    fn check_bundle_domains(&mut self, m: &ModuleDecl) {
            ...
            ExprKind::Path(_) => match self.res.resolutions.get(&e) {
                Some(&def) => {
                    if self.bidir_ports.contains(&def) {
                        return self.bidir_read(def, expr.span);
                    }
                    self.def_domain(def)
                }
            ...
            ExprKind::Call { callee, args } => {
                ...
                if let Some(kind) = self.builtin_of(callee) {
                    if matches!(kind, BuiltinKind::Sync | BuiltinKind::Sync3) {
                        ...
                        let d = self.sync_domain(&args, expr.span);
